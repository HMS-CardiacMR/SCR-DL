...
class BDCartesianSSDULoader(Dataset):
    ...
    def __init__(self, datapath,sno):
        #self.data_path = glob.glob(os.path.join(datapath, 'test_subject1.mat'))
        #self.data_path = glob.glob(os.path.join(datapath, 'test_subject'+str(sno)+'.mat'))
        self.data_path = glob.glob(os.path.join(datapath, 'subject'+str(sno)+'.mat'))
    ...
    def __getitem__(self, index):
        ...
        acs_filt = 12 #means 16x2 x 16x2 acs region, 16 for 46Acs, 12 for 24 ACS
        reg = 1e-3 #spirit weight regularization, original 1e-2
